...
import { updateAuthor } from "../../../functions/forAuthor";
...
export const EditAuthorData = (props: Props) => {
  ...
  useEffect(() => {
    ...
    props.columns.forEach((column) => {
      if (column.field !== "id") {
        defaultValues[column.field] = (props.author as any)?.[column.field] || "";
      }
    });
    setUpdatedData(defaultValues);
  }, [props.columns, props.author]);

  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();

    try {
      await updateAuthor(props.id, updatedData, accessToken);
