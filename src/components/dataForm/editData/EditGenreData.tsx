...
import { updateGenre } from "../../../functions/forGenre";
...
export const EditGenreData = (props: Props) => {
  ...
  useEffect(() => {
    ...
    props.columns.forEach((column) => {
      if (column.field !== "id") {
        defaultValues[column.field] = (props.genre as any)?.[column.field] || "";
      }
    });
    setUpdatedData(defaultValues);
  }, [props.columns, props.genre]);

  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();

    try {
      await updateGenre(props.id, updatedData, accessToken);
